...
ELECTIVE_COURSES = {
    "CPE": {
        "CPE 453","CPE 520","CPE 521","CPE 536","CPE 538","CPE 664","CPE 684"
    },
    "CS": {
        "CS 533","CS 539","CS 555","CS 556","CS 558","CS 568",
        "CS 572","CS 665","CS 674","CS 676","CS 677","CS 678","CS 757"
    },
    "EE": {
        "EE 435","EE 436","EE 437","EE 455","EE 461","EE 465",
...
def generate_html_site(df_calendar, output_path, rules=[]):
    ...
    for dept, group in df_calendar.groupby('Department'):
        ...
        # HTML generation for individual dept calendars
...
# Elective Department Rule
st.markdown("###### Rule 03: Prevent overlap of *all* electives in a department")
with st.form("elective_dept_form"):
    dept_elective = st.selectbox("Department", ["CPE","CS","EE"], key="elective_dept_input")
    add_elective_rule = st.form_submit_button("➕ Add Elective Dept Rule")
...
if "elective_depts" not in st.session_state:
    st.session_state.elective_depts = []

if add_elective_rule:
    st.session_state.elective_depts.append(dept_elective)

if st.session_state.elective_depts:
    st.markdown("#### Elective‐Dept Non-Clash Rules:")
    for i, d in enumerate(st.session_state.elective_depts):
        cols = st.columns([6,1])
        with cols[0]:
            st.write(f"• No two electives in **{d}** may overlap")
        with cols[1]:
            if st.button("🗑️", key=f"remove_elective_{i}"):
                st.session_state.elective_depts.pop(i)
                st.rerun()
...
st.markdown("""
	...
	<p style="font-size: 12px; color: #FFF;">Version: 1.2</p>
    </div>
